<?php
    ...
    $returnData = [];
     ...
     //Tarkistetaan oliko Requesti PUT
     if($_SERVER["REQUEST_METHOD"] != "PUT") {
        $returnData = msg(0,404,'Sivua ei löytynyt.');

        //jos oli, niin katsotaan onko inputeissa tavaraa
     } elseif(!isset($data->id) || !isset($data->title) 
            || !isset($data->description) || !isset($data->price)
            || !isset($data->image) || !isset($data->gtin)
            || empty(trim($data->id)) || empty(trim($data->title))
            || empty(trim($data->description)) || empty(trim($data->price))
            || empty(trim($data->image)) || empty(trim($data->gtin)))
    {
        $fields = ['fields' => ['id', 'title', 'description', 'price', 'image', 'gtin' ]];
        ...
        if($item->updateItem()){
            $returnData = msg(1,201,'Tuote päivitetty!');
        } else{
